 ...
 async registerRestaurantByAdmin(restaurantData) {
  ...
  async updateAllRestaurantFields(restaurantId, updateData) {
    try {
      ...
      if (updateData.email !== undefined) {
        ...
        const existingRestaurant = await restaurantRepository.findByEmail(
          updateData.email
        );
        if (
          existingRestaurant &&
          existingRestaurant.restaurant_id !== restaurantId
        ) {
          throw new Error("Email already exists");
        }
      ...
      const updatedRestaurant = await restaurantRepository.findByRestaurantId(
        restaurantId
      );
      ...
      return {
        ...
        restaurant: {
          ...
          cuisine:
            typeof updatedRestaurant.cuisine === "string"
              ? JSON.parse(updatedRestaurant.cuisine)
              : updatedRestaurant.cuisine,
          menu:
            typeof updatedRestaurant.menu === "string"
  ...
  async login(email, password) {
    try {
      ...
      // Verify password
      const isPasswordValid = await restaurantRepository.verifyPassword(
        password,
        restaurant.password
      );
      if (!isPasswordValid) {
  ...
  async changePassword(email, oldPassword, newPassword) {
    try {
      ...
      // Verify old password
      const isOldPasswordValid = await restaurantRepository.verifyPassword(
        oldPassword,
        restaurant.password
      );
  ...
  async updateSettings(restaurantId, settings) {
    try {
      ...
      // Payment toggle: support cash payments, validate boolean
      if (settings.cash_payment_enabled !== undefined) {
        if (typeof settings.cash_payment_enabled !== "boolean") {
          throw new Error("cash_payment_enabled must be a boolean");
        }
        validSettings.cash_payment_enabled = settings.cash_payment_enabled;
      }

      // Ensure at least one service is enabled if restaurant is online
      if (validSettings.is_online === true) {
        const currentSettings = await restaurantRepository.getSettings(
          restaurantId
        );
        const deliveryWillBeEnabled =
          validSettings.delivery_enabled ?? currentSettings.delivery_enabled;
        const takeawayWillBeEnabled =
          validSettings.takeaway_enabled ?? currentSettings.takeaway_enabled;

        if (!deliveryWillBeEnabled && !takeawayWillBeEnabled) {
          throw new Error(
            "At least one service (delivery or takeaway) must be enabled when restaurant is online"
          );
        }
      }

      // Ensure at least one payment method remains enabled (card or cash)
      if (
        validSettings.card_payment_enabled !== undefined ||
        validSettings.cash_payment_enabled !== undefined
      ) {
        const currentSettings = await restaurantRepository.getSettings(
          restaurantId
        );
        const cardWillBeEnabled =
          validSettings.card_payment_enabled ??
          currentSettings.card_payment_enabled;
        const cashWillBeEnabled =
          validSettings.cash_payment_enabled ??
          currentSettings.cash_payment_enabled;

        if (!cardWillBeEnabled && !cashWillBeEnabled) {
          throw new Error(
            "At least one payment method (card or cash) must be enabled"
          );
        }
      }
      ...
      const updatedSettings = await restaurantRepository.getSettings(
        restaurantId
      );
    ...
    try {
      ...
      if (isOnline) {
        ...
        if (!settings.delivery_enabled && !settings.takeaway_enabled) {
          throw new Error(
            "Please enable at least one service (delivery or takeaway) before going online"
          );
        }
      }
  ...
  async toggleService(restaurantId, serviceType, enabled) {
    try {
      ...
      if (!validServices.includes(serviceType)) {
        throw new Error(
          'Invalid service type. Must be "delivery" or "takeaway"'
        );
      }
      ...
      if (!enabled && restaurant.is_online) {
        ...
        const otherService =
          serviceType === "delivery" ? "takeaway_enabled" : "delivery_enabled";

        if (!currentSettings[otherService]) {
          throw new Error(
            `Cannot disable ${serviceType}. At least one service must be enabled when restaurant is online`
          );
        }
      }
